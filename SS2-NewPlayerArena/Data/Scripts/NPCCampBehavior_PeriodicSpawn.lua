...
local RESET_COUNT = script:GetCustomProperty("RespawnCount")
...
function Tick(deltaTime)
	...
	if
		(currentState == 0 and stateElapsedTime >= INITIAL_DELAY) or
			(currentState == 1 and stateElapsedTime >= RESPAWN_COOLDOWN)
	 then
		if CAMP_SPAWNER.context.minionCount <= RESET_COUNT then
			Reset()
