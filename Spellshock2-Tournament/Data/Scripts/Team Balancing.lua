...
function ComputePlayerValue(player)
	...
	if winRateValue then
		value = WIN_RATE_COEFFICIENT * weightedWinRate
	...
	print(
		"[Balance] Player " ..
			player.name ..
				...
				", winRateValue = " ..
					tostring(weightedWinRate) .. "->" .. tostring(winRateValue) .. ", totalValue = " .. tostring(value)
	)
...
function DoRebalance(playerToIgnore)
	...
	while i < 64 do
		...
		if Object.IsValid(player1) and Object.IsValid(player2) then
...
function OnGameStateChanged(oldState, newState)
	if newState == ABGS.GAME_STATE_LOBBY and oldState ~= ABGS.GAME_STATE_LOBBY then
		while not ABGS.GetTimeRemainingInState() do
			Task.Wait()
		end -- Wait for the state to have a timer
