		...
		if Timer < 0 then
			...
			if ClassEquipmentReference then
				...
				if classID and classID > 0 then
					...
					classEquipment:SetCustomProperty("OID", oId)
					classEquipment:SetCustomProperty("QID", qId)
					classEquipment:SetCustomProperty("EID", eId)
					classEquipment:SetCustomProperty("RID", rId)
					classEquipment:SetCustomProperty("TID", tId)
				elseif costumeID then
					classID = _Owner:GetResource("CLASS_MAP") 
					classID = classID > 0 and classID or 1
					local oId = GetCurrentCosmeticId(_Owner, classID, 8)
					classEquipment:SetCustomProperty("OID", oId)
					classEquipment:SetCustomProperty("ClassID", classID)
				end
