...
local function BuildAchievmentPanels()
    ...
    for _, achievement in pairs(ACH_API.GetAchievements()) do
        ...
        panel:GetCustomProperty("PROGRESS_TEXT"):WaitForObject().text = tostring(ACH_API.FormatInt(currentResource == 0 and 0 or currentResource > 1 and currentResource - 1)) .. " / " .. tostring(ACH_API.FormatInt(CoreMath.Round(requiredResource - 1, 0)))
