local EQUIPMENT = script.parent:GetCustomProperty("Equipment"):WaitForObject()
...
function Tick(deltaTime)
    if Object.IsValid(EQUIPMENT) then
        ...
        if owner and owner.serverUserData.healthRegenSpeed then
            if owner.hitPoints < owner.maxHitPoints and
            time() > lastTime then
                local healthRegenSpeed = owner.serverUserData.healthRegenSpeed
                owner.hitPoints = owner.hitPoints + healthRegenSpeed
            end
        end
    end
end
