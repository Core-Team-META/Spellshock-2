...
function AbilityTick(ability, deltaTime)
    if SHOOT_ABILITY:GetCurrentPhase() == AbilityPhase.CAST then
        local chargeTime = SHOOT_ABILITY.castPhaseSettings.duration - SHOOT_ABILITY:GetPhaseTimeRemaining()
        WEAPON.serverUserChange.chargeTime = chargeTime
    end
end
...
function OnCastAbility(ability)
    WEAPON.serverUserChange.chargeTime = 0
end
